...
public class myPluginWiki extends JavaPlugin {
	...
	public static String[] enable_messages = { "WRITE SOME ENABLE MESSAGES!" }, disable_messages = { "WRITE SOME DISABLE MESSAGES!" };
	...
	private static int[] must_be_attached_bottom_only_IDs = { 6, 26, 27, 28, 31, 32, 37, 38, 39, 40, 55, 59, 63, 64, 66, 70, 71, 72, 78, 81, 83, 93, 94, 104, 105, 111, 115,
			132, 140, 147, 148, 149, 150, 157 }, must_be_attached_can_be_sideways_IDs = { 50, 65, 68, 69, 75, 76, 77, 96, 106, 127, 131 }, can_be_broken_by_liquids_IDs = { 0,
			27, 28, 30, 31, 32, 37, 38, 39, 40, 50, 51, 55, 59, 66, 69, 75, 76, 78, 93, 94, 104, 105, 106, 115, 127, 131, 132, 140, 141, 142, 144, 149, 150, 157 },
			lockable_portal_IDs = { 64, 69, 70, 72, 77, 96, 107, 143, 147, 148 }, lockable_container_IDs = { 23, 54, 58, 61, 62, 84, 95, 116, 117, 130, 138, 145, 146, 154,
					158 };
	...
	public void onEnable() {
		...
		console.sendMessage(ChatColor.DARK_PURPLE + enable_message);
		for (Player player : getServer().getOnlinePlayers())
			if (player.isOp())
				player.sendMessage(ChatColor.DARK_PURPLE + enable_message);
	}

	public void onDisable() {
		// done disabling
		String disable_message = disable_messages[(int) (Math.random() * disable_messages.length)];
		console.sendMessage(ChatColor.DARK_PURPLE + disable_message);
		for (Player player : getServer().getOnlinePlayers())
			if (player.isOp())
				player.sendMessage(ChatColor.DARK_PURPLE + disable_message);
	}
	...
	public static String getItemName(int id, int data, boolean give_data_suffix, boolean singular, boolean without_article) {
		...
		// if the item is singular and we want no artcile, remove the preexisting article
		if (singular && without_article)
			item = item.substring(item.split(" ")[0].length());
		// if the item is plural and we want an artciel, just add a "some" to the beginning
	...
	public static boolean isLockable(int id, Boolean has_inventory) {
		if (has_inventory == null || has_inventory)
	...
	public static boolean isLockable(Block block, Boolean has_inventory) {
		return isLockable(block.getTypeId(), has_inventory);
	}
}